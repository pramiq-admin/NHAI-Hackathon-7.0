...
- [x] `py -c "import torch, onnx, tensorflow, cv2"` all import without error
...
- [x] GitHub repo created, README placeholder pushed
- [x] Firebase project created with Remote Config enabled
- [ ] Play Console account verified (Play Integrity API will be needed Day 9)
- [x] Ed25519 keypair generated for `.tflite` signing
...
### Acceptance Criteria
- [x] App builds and installs on test phone
- [x] Camera preview shows live feed
...
### Acceptance Criteria
- [x] EdgeFace INT8 model is ~2 MB on disk
...
- [x] ⭐ BioHashing: stored template cannot be reversed to original embedding (validate with `np.dot(M.T, hashed) ≠ emb`)
- [x] ⭐ BioHashing: regenerating salt per enrollment produces different hash for same face (unlinkability)
- [x] ⭐ MagFace quality gate: blurry/dark capture rejected before match runs (latency saving visible in logs)
